    ...
    <div className="word-section">
      <div className="word-row">
        ...
        <div className="word-box">
          <span className="word-text">{word || <span className="placeholder">start signing…</span>}</span>
      ...
      {sentence && (
        <div className="sentence-row">
          ...
          <div className="sentence-box">
            ...
            <button className="btn-clear-sentence" onClick={onClearSentence} title="Clear sentence">✕</button>
          </div>
        </div>
      )}
    </div>
...
// ─── StatusBar ───────────────────────────────────────────────────────────────
const STATUS_CONFIG = {
  open:       { color: "#00e5a0", label: "Connected",   dot: "●" },
  connecting: { color: "#f7c948", label: "Connecting…", dot: "◌" },
  closed:     { color: "#ff6b6b", label: "Disconnected", dot: "●" },
  error:      { color: "#ff6b6b", label: "Error",        dot: "●" },
};
...
export function StatusBar({ wsStatus, isActive }) {
  ...
  return (
    <div className="status-bar">
      <span className="status-dot" style={{ color: cfg.color }}>{cfg.dot}</span>
  ...
  return (
    <div className="control-panel">
      ...
      <div className="control-group">
        ...
        <button className="btn-ctrl" onClick={onBackspace} title="Backspace">⌫</button>
        <button className="btn-ctrl" onClick={onClear}    title="Clear word">CLR</button>
      </div>
      <p className="kbd-hint">Keyboard: <kbd>Space</kbd> <kbd>⌫</kbd> <kbd>Enter</kbd></p>
    </div>
  );
}
